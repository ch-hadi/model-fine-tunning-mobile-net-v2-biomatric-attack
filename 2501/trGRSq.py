...
# -----------------------------
DATASET_NAME = "PLUS"  # Option: "SCUT", "PLUS", "IDIAP"
...
# -----------------------------
# 3. METRICS & TRAINING (SqueezeNet with Transfer Learning)
# -----------------------------
...
def train_and_eval(train_records, test_records):
    ...
    train_loader = DataLoader(PADDataset(train_records, transform), batch_size=BATCH_SIZE, shuffle=True, num_workers=4, pin_memory=True)
    test_loader  = DataLoader(PADDataset(test_records,  transform), batch_size=BATCH_SIZE, shuffle=False, num_workers=4, pin_memory=True)
    
    # ----- SqueezeNet1_1 with Transfer Learning -----
    model = models.squeezenet1_1(weights=models.SqueezeNet1_1_Weights.IMAGENET1K_V1)
    # Replace the final conv layer in classifier for 2 classes (real/spoof)
    ...
    print(f"Training on {len(train_records)} images | Testing on {len(test_records)} images")
...
# -----------------------------
# 4. CORE LOGIC FOR STEPS 3, 4, 5 (Folds & Data Logic Same as Original)
# -----------------------------
def run_experiment_step(real_records, syn_records, step_num):
    print(f"\n{'='*20} RUNNING {DATASET_NAME} - STEP {step_num} {'='*20}")
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(real_records, labels, subjs), 1):
        ...
        if step_num == 3:
            ...
            for r in real_train:
                if r["subject"] in s2_subs: 
                    train_final.append(r)
                if r["subject"] in s3_subs and r["label"] == 0: 
                    train_final.append(r)
            for s in syn_records:
                if s["subject"] in s3_subs: 
                    train_final.append(s)
            ...
            for r in real_train:
                if r["subject"] in s2_subs: 
                    train_final.append(r)
                if r["subject"] in others and r["label"] == 0: 
                    train_final.append(r)
            for s in syn_records:
                if s["subject"] in others: 
                    train_final.append(s)
    ...
    print(f"\n>>> FINAL SUMMARY {DATASET_NAME} STEP {step_num}: Avg ACER = {avg_acer:.4f}")

if __name__ == "__main__":
    real_data = build_real_index(REAL_ROOT, DATASET_NAME)
    syn_data = build_syn_index(SYN_ROOT, DATASET_NAME)
    print(f"Loaded {DATASET_NAME}: {len(real_data)} Real images, {len(syn_data)} Synthetic images")
